...
},{"bn.js":1,"strip-hex-prefix":5}],5:[function(require,module,exports){
...
},{"is-hex-prefixed":2}],6:[function(require,module,exports){
(function (Buffer){
const utils = require('./index.js');
const coder = require('./utils/index.js');
const uint256Coder = coder.uint256Coder;
const coderBoolean = coder.coderBoolean;
const coderFixedBytes = coder.coderFixedBytes;
const coderAddress = coder.coderAddress;
const coderDynamicBytes = coder.coderDynamicBytes;
const coderString = coder.coderString;
const coderArray = coder.coderArray;
const paramTypePart = coder.paramTypePart;
...
function randomInput(method) {
  var parts = [];
  var types = coder.getKeys(method.inputs, 'type')

  types.forEach(function(type, index) {
    var coder = getParamCoder(type);
    parts.push({dynamic: coder.dynamic, value: coder.encode(randomType(type))});
  });
  ...
  parts.forEach(function(part, index) {
    if (part.dynamic) {
      coder.uint256Coder.encode(dynamicOffset).copy(data, offset);
      offset += 32;
...
utils.sign = function sign(method) {
	const signature = `${method.name}(${coder.getKeys(method.inputs, 'type').join(',')})`;
	const signatureEncoded = `0x${(new Buffer(coder.keccak256(signature), 'hex')).slice(0, 4).toString('hex')}`;
  

	return signatureEncoded;
}

utils.random = function random(method) {
	const paramsEncoded = randomInput(method);
	if(method.sign != undefined)
		return `${method.sign}${paramsEncoded}`;

	const signatureEncoded = utils.sign(method);
...
//console.log('SimpleStoreABI=', SimpleStoreABI);
console.log('random(SimpleStoreABI[0])=', random(SimpleStoreABI[0]));
console.log('random(SimpleStoreABI[1])=', random(SimpleStoreABI[1]));
console.log('random(SimpleStoreABI[2])=', random(SimpleStoreABI[2]));
//random(SimpleStoreABI[0]);
*/


window.ethabi = utils;
}).call(this,require("buffer").Buffer)
},{"./index.js":7,"./utils/index.js":8,"buffer":10}],7:[function(require,module,exports){
(function (Buffer){
/* eslint-disable */
...
},{"./utils/index.js":8,"buffer":10}],8:[function(require,module,exports){
(function (Buffer){
...
},{"bn.js":1,"buffer":10,"js-sha3":3,"number-to-bn":4}],9:[function(require,module,exports){
'use strict'
